...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mTextSelectSize = ta.getDimension(R.styleable.SlidingTabLayout_tl_textSelectSize, mTextsize);//未设置时跟默认未选中字体大小一致
...
//            mIndicatorRect.right = (int) (mIndicatorRect.left + mIndicatorWidth);
            ...
            if (this.mCurrentTab < mTabCount - 1) {
                ...
                float rightX;
                float nextRightX;
                leftX = currentTabView.getLeft() + (currentTabView.getWidth() - mIndicatorWidth) / 2;
                nextLeftX = nextTab.getLeft() + (nextTab.getWidth() - mIndicatorWidth) / 2;
                rightX = currentTabView.getLeft() + (currentTabView.getWidth() + mIndicatorWidth) / 2;
                nextRightX = nextTab.getLeft() + (nextTab.getWidth() + mIndicatorWidth) / 2;
                float leftIndicator, rightIndicator;
                leftIndicator = leftX + (nextLeftX - leftX) * mStartInterpolator.getInterpolation(mCurrentPositionOffset);
                rightIndicator = rightX + (nextRightX - rightX) * mEndInterpolator.getInterpolation(mCurrentPositionOffset);
                mIndicatorRect.left = (int) (leftIndicator);
                mIndicatorRect.right = (int) (rightIndicator);
            } else {
                mIndicatorRect.left = (int) indicatorLeft;
                mIndicatorRect.right = (int) (mIndicatorRect.left + mIndicatorWidth);
            }
